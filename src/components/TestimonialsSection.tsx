    ...
    <section id="avis" className="py-16 sm:py-24 bg-caffeine-dark border-b border-caffeine-cardBorder relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-caffeine-subtle text-sm sm:text-base mt-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {REVIEWS.map((review) => (
            <div
              key={review.id}
              className="p-6 rounded-3xl bg-white border border-caffeine-cardBorder hover:border-caffeine-gold/50 transition-all duration-300 flex flex-col justify-between shadow-[0_4px_20px_rgba(180,130,80,0.07)] hover:shadow-[0_12px_32px_rgba(180,130,80,0.13)] relative group"
            >
              <div>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-1 text-caffeine-gold">
                    {[...Array(review.rating)].map((_, i) => (
                      <Star key={i} className="w-4 h-4 fill-caffeine-gold" />
                    ))}
                  </div>
                  <Quote className="w-6 h-6 text-caffeine-gold/30 group-hover:text-caffeine-gold/60 transition-colors" />
                </div>
                ...
                {/* Review Text */}
                <p className="text-xs sm:text-sm text-caffeine-cream/90 italic leading-relaxed mb-6 font-sans">
                  &ldquo;{review.comment}&rdquo;
                </p>
              </div>

              {/* Author Footer */}
              <div className="pt-4 border-t border-caffeine-cardBorder/60 flex items-center justify-between">
                <div>
        ...
        {/* Trust Badges Bar */}
        <div className="mt-14 p-6 rounded-2xl bg-white border border-caffeine-cardBorder flex flex-wrap items-center justify-around gap-6 text-center text-xs text-caffeine-subtle shadow-[0_4px_20px_rgba(180,130,80,0.06)]">
